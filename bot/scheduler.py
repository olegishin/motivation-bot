# 07 - bot/scheduler.py
# 07 - bot/scheduler.py - 26.01.2026
# 07 - bot/scheduler.py
# 07 - bot/scheduler.py
# ✅ ULTIMATE VERSION (28.01.2026)
# ✅ КРИТИЧЕСКИЙ ФИКС: Рассылка строго в 03:00 UTC (1 раз в сутки) + Защита от дублей
# ✅ ИНТЕГРАЦИЯ (Аудит 1.d): Напоминания из challenges.py (16:00 и +1 час)
# ✅ ИНТЕГРАЦИЯ (Аудит 3, 4): Логика "Дня тишины" и маркетинговых сообщений
# ✅ СОХРАНЕНО: Функции безопасности Олега, бэкапы, Smart Ban и логирование
# 07 - bot/scheduler.py
# ✅ ВЫДАНО ЦЕЛИКОМ ДЛЯ ЗАМЕНЫ
# ✅ ПОЛНАЯ СИНХРОНИЗАЦИЯ: 3+1+3 + Маркетинг (8, 12, 15, 18) + Челленджи
# ✅ КРИТИЧЕСКИЙ ФИКС: Защита от дублей через метку "Дата_Час" (Стоп спам!)
# ✅ СОХРАНЕНО: Безопасность Олега, Бэкапы в 03:05, Smart Ban, асинхронность
...
# --- 🛡️ ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ БЕЗОПАСНОСТИ (ОТ ОЛЕГА) ---
...
async def centralized_broadcast_job(bot: Bot, static_data: dict):
    ...
    for chat_id_str, user_data in users_db.items():
        try:
            ...
            # Формат: "2026-01-28_08" - если уже слали в этот час, пропускаем
...
async def backup_job(bot: Bot):
    """Ежедневный бэкап базы в 03:05 UTC (Твоя логика)."""
